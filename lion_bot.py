...
# TODO 定时更新   录用户平均更新时间  优化结构
# TODO


class lion_bot:
    def __init__(self):
        self.update_amount = 10
        self.update_gap = 12
        self.last_update_time = 0
        TOKEN = configure.configure().TOKEN
        self.MY_CHAT_ID = 686839482
        self.GROUP_ID = -633034091
        self.CHANNEL_ID = -1001592822937

        self.updater = Updater(TOKEN)
        self.updater.dispatcher.add_handler(MessageHandler(Filters.chat_type.channel, self.follow_user_filter))
        self.updater.dispatcher.add_handler(CallbackQueryHandler(self.process_query))
        self.updater.dispatcher.add_handler(CommandHandler("delete_activity_message", self.delete_activity_message))
    ...
    def con_thread(self):
        ...
        while True:
            ...
            if self.next_time is None:
                ...
                last_time.sort()
                self.next_time = last_time[self.update_amount] + self.update_gap * 3600
            ...
            if len(self.messages) > 0:
                for message in self.messages:
                    message['id'] = message['id'][message['id'].find('status/') + 7:message['id'].find('status/') + 26]
                    print(message['id'])
                    self.cur.execute(
                        "UPDATE data SET message_status=\'%s\' where status_id=\'%s\'" % (
    ...
    def add_follow_user(self, update, context):
        if update.effective_chat.id == self.MY_CHAT_ID or update.effective_chat.id == self.GROUP_ID:
            try:
    ...
    def update_link(self):
        if self.update_list:
            if len(self.update_list) > 0:
                ...
                for i in self.update_list:
                    result += i + '\n'
                self._send_message_to_group(result)
                message = twitter_spider(self.con, self.cur, self.update_list).process()
                self.update_list = None
                for i in message:
                    self.activity_messages.append(self._send_message_for_check(i))
                self._send_message_to_group("更新完成")
                self.next_time = None
                self.last_update_time = time.time()
        else:
            self._send_message_to_group("没有需要更新的链接")

    def delete_activity_message(self, update, context):
        if update.effective_chat.id == self.MY_CHAT_ID:
            if self.activity_messages:
                for i in self.activity_messages:
                    try:
                        self.bot.delete_message(chat_id=686839482, message_id=i['message_id'])
                    except Exception:
                        pass
    ...
    def process_query(self, update, context):
        query = update.callback_query
        if query.data == 'forward':
            self._send_message_to_channel(query.message.text)
            self.messages.append({'status': 'processed', 'id': query.message.text})
        elif query.data == 'delete':
            self.messages.append({'status': 'deleted', 'id': query.message.text})
        query.delete_message()

    # 处理频道消息

    def follow_user_filter(self, update: Update, context: CallbackContext):
        message = update.channel_post
        if message.text.__contains__("status") and message.text.__contains__("twitter"):
            user = message.text[message.text.find(".com/") + 5: message.text.find("/status/")]
            self.bot.delete_message(message_id=message.message_id, chat_id=message.chat_id)
            result = self._add_follow_user(user)
            self._send_message_to_group(result)
    ...
    def _send_message_to_channel(self, text):
        self.bot.send_message(chat_id=self.CHANNEL_ID, text=text)

    def _send_message_to_group(self, text):
        self.bot.send_message(chat_id=self.GROUP_ID, text=text)

    def _send_message_to_me(self, text):
        self.bot.send_message(chat_id=self.MY_CHAT_ID, text=text)

    def _send_message_for_check(self, text):
        button0 = InlineKeyboardButton(text="Forward", callback_data="forward")
        button1 = InlineKeyboardButton(text="Delete", callback_data="delete")
        keyboard = InlineKeyboardMarkup([[button0, button1]])
        return self.bot.send_message(chat_id=self.GROUP_ID, text=text, reply_markup=keyboard)
